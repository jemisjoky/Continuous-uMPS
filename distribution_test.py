...
def compute(input, embedding, domain):
	...
	my_mps = ProbMPS(sequence_len, input_dim, bond_dim, complex_params, embed_fun=embedding, domain=domain)
...
batchInputs=torch.stack([torch.tensor(X1.flatten()), torch.tensor(X2.flatten())], dim=-1).float().transpose(0, 1)
